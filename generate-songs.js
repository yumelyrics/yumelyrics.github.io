...
<div id="login-gate">
  ...
  <div id="login-gate-sub">Lirik bisa dibaca tanpa login. Login dengan Google untuk copy lirik &amp; ikut diskusi di kolom komentar</div>
...
<section class="lyrics-section" id="lyrics">
  ...
  <main class="lyrics-main">
    <div class="lyrics-controls">
      ...
      <button class="ctrl-pill" id="copy-lyric-btn" onclick="doCopyLyric()" style="margin-left:auto;opacity:.38;cursor:not-allowed" disabled title="Login dulu untuk copy lirik">
...
<div id="login-toast" role="status" aria-live="polite">
  <div class="lt-inner">
    ...
    <div class="lt-body">
      ...
      <div class="lt-desc">Setelah login, <strong>tombol copy</strong> di sebelah terjemahan aktif untuk menyalin lirik. Komentar lewat GraphComment di bawah halaman.</div>
...
function updateCopyGate() {
  ...
  if (gate) {
    gate.style.display = _currentUser ? 'flex' : 'none';
  }
  if (!btn) return;
  if (!_currentUser) {
    btn.disabled = true;
  ...
  if (_isAdmin) {
    ...
    if (sub) sub.innerHTML = '<span style="color:var(--gold)">👑 Admin — copy tanpa batas.</span>';
    return;
  }
  if (_isBanned) {
    btn.disabled = true;
  ...
  if (label) label.textContent = 'Copy';
  if (sub) sub.textContent = 'Login aktif — kamu bisa meng-copy lirik lagu ini.';
}
...
async function applyAuthLoggedIn(user, gen){
  ...
  if (user) {
    ...
    if (_justLoggedIn && !_isBanned && !_isAdmin) {
      _justLoggedIn = false;
      setTimeout(() => {
