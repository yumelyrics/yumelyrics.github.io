...
/* ── NAV ── */
nav{position:sticky;top:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:1.4rem 3rem;background:rgba(245,240,234,.9);backdrop-filter:blur(20px);border-bottom:1px solid rgba(10,8,18,.08)}
...
.nb{font-size:.68rem;font-weight:600;letter-spacing:.2em;text-transform:uppercase;color:var(--ash);text-decoration:none;transition:color .2s;background:none;border:none;font-family:var(--sans);cursor:pointer;padding:0}
.nb:hover,.nb.on{color:var(--ink)}
...
<nav>
  <a class="nav-brand" href="/index.html">
    <span class="nav-brand-jp">夢字幕</span>
...
<section class="dcm-sec">
  <div class="dcm-intro">
    ...
    <div>
      ...
      <div class="dcm-form">
        <div class="cmrow">
          <input class="cmi" id="cm-n" placeholder="Nama (opsional)">
...
import { getFirestore, collection, addDoc, query, getDocs, where, orderBy }
...
window.postReply = async parentId => {
  ...
  if(!t) return;
  const n=document.getElementById('srn-'+parentId).value.trim();
  try {
    await addDoc(collection(db,'story_comments'),{
      storyId:STORY_ID, parentId,
      name:n||'Anonim', text:t,
...
window.postCm = async () => {
  ...
  try {
    await addDoc(collection(db,'story_comments'),{
      storyId:STORY_ID, parentId:null,
      name:n||'Anonim', text:t,
      date:new Date().toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'}),
      ts:Date.now(), isAdmin:false
    });
